            ...
            if(typeof(data.issues[0].custom_fields) != "undefined" && data.issues[0].custom_fields !== null) {
              ...
              $.each(data.issues[0].custom_fields, function(i, custom_field) {
                console.log(custom_field);
    				...
    				if(typeof(data.issues[0].parent) != "undefined" && data.issues[0].parent !== null && (!f_assigned_to)) //есть родитель и нет назначенного
    					getTreeIssues(data.issues[0].parent.id);
    				else
    					if (f_assigned_to) {
    						$('#issue_priority_id').attr("disabled", true); 
                $('[name="issue[custom_field_values][6]"]').attr("disabled", true);
                $('[name="issue[custom_field_values][6]"]').attr("disabled", true);
                $('#issue_custom_field_values_7').attr("disabled", true); 
              }  
    			}
			}
		});
